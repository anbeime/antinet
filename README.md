...
#### 🤖 智能知识库助手
- 右下角聊天机器人图标，点击打开对话窗口
- 基于四色卡片知识库（事实/解释/风险/行动）回答问题
- 支持拖拽移动对话框到任意位置
- 自动滚动、自动聚焦、实时响应

#### ⚡ NPU 性能优化
- 启用 **BURST 性能模式** 提升推理速度
- 延迟从实测 **2840.5ms** 优化目标至 **<500ms**
- 已添加延迟监控和报警机制
...
**Antinet智能知识管家**是一款部署于骁龙AIPC的端侧智能数据工作站。通过集成NPU加速的轻量化大模型(Qwen2.0-7B-SSD),实现:

- 🗣️ **自然语言驱动的数据查询** - "分析上个月的销售趋势"
- 📊 **自动数据分析与可视化** - 模块化智能处理，自动生成四色卡片与交互图表
- 🎴 **四色卡片知识沉淀** - 基于卢曼卡片盒方法论，实现知识自动关联与图谱构建
- 🔒 **数据不出域** - 所有处理在骁龙AIPC端侧完成，保障数据安全
- ⚡ **NPU加速推理** - 骁龙Hexagon NPU赋能，任务智能拆解，BURST性能模式
- 🤖 **智能知识库助手** - 基于四色卡片的问答系统，支持实时查询
...
- **智能问答**: 知识库助手自动解答系统使用、NPU性能、风险等问题
...
| 环境 | Python 版本 | 架构 | 用途 |
|------|-------------|------|------|
| `venv_arm64` | 3.12.10 | ARM64 | **推荐**：针对 ARM64 NPU 优化，支持 NPU 加速 |
| `venv` | 3.12.10 | x64 | 通用 x64 环境，用于开发和测试 |
| 系统 Python | 3.12.10 | 系统默认 | 已安装依赖，可直接使用 |

### 使用建议

1. **NPU 开发**：使用 `venv_arm64` 环境（`start_backend.bat` 自动优先选择）
2. **普通开发**：使用 `venv` 环境
3. **快速启动**：直接使用系统 Python（已安装所有依赖）
...
# 手动激活虚拟环境
venv_arm64\Scripts\activate.bat  # ARM64 环境
venv\Scripts\activate.bat        # x64 环境
```
...
**常见问题**:
-no 推理延迟 > 500ms → 检查是否使用 NPU，启用 BURST 模式
-no 模型加载失败 → 检查模型文件是否存在，验证 QNN 库路径
-no DLL 加载失败 → 设置 PATH 环境变量，安装 Visual C++ Redistributable
...
**联系方式**: [通过比赛官方渠道]
**最后更新**: 2026年1月11日
基于您提供的**Antinet项目方案**和**文档资料**，我为您设计了一个**包含8个智能体的协同架构**，并充分考虑了**高通骁龙平台的端侧部署要求**。

---

### **Antinet智能知识管家 — Multi-Agent 协同架构方案**

本方案将您原有的“数据分析 + 卡片沉淀”流程，升级为一个**由8个专家智能体自主协同、在端侧高效执行的智能系统**。
...
flowchart TD
    ...
    U --> O[协调与控制中心 Agent (锦衣卫总指挥使)<br>Orchestrator]
    
    O --> P[数据感知与预处理 Agent (密卷房)<br>Preprocessor]
    P -- “结构化查询条件” --> O
    ...
    subgraph C_inner [四色卡片生成链]
        C1[🔵 事实卡片生成器 (通政司)<br>Fact Generator]
        C2[🟢 解释卡片生成器 (监察院)<br>Interpreter]
        C3[🟡 风险卡片生成器 (刑狱司)<br>Risk Detector]
        C4[🔴 行动卡片生成器 (参谋司)<br>Action Advisor]
    end
    ...
    C1 -- “事实卡片” --> M[记忆与知识库 Agent (太史阁)<br>Memory]
    C2 -- “解释卡片” --> M
    C3 -- “风险卡片” --> M
    C4 -- “行动卡片” --> M
    ...
    O --> R[报告合成与呈现 Agent (驿传司)<br>Reporter]
    R --> U_Out[最终分析报告与交互界面]
```
