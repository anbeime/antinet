...
# 新的菜单代码
new_menu = '''<div className="hidden md:flex items-center space-x-1">
            {/* 概览 */}
            <button
...
# 查找并替换菜单代码
# 使用正则表达式匹配整个菜单区域
pattern = r'<div className="hidden md:flex items-center space-x-6">.*?</div>\s*</div>\s*<div className="flex items-center space-x-4">'

match = re.search(pattern, content, re.DOTALL)
if match:
    # 替换菜单代码
    new_content = content[:match.start()] + new_menu + '\n          </div>\n          <div className="flex items-center space-x-4">' + content[match.end():]
    ...
    print("❌ 未找到菜单代码")
    # 尝试查找菜单代码的位置
    if 'hidden md:flex items-center space-x-6' in content:
        print("找到 'hidden md:flex items-center space-x-6'")
    if 'hidden md:flex items-center space-x-1' in content:
        print("找到 'hidden md:flex items-center space-x-1'")
