...
def test_cpu_vs_npu_comparison():
    """测试 CPU vs NPU 性能对比（模拟）"""
    print("\n" + "=" * 70)
    print("测试 4: CPU vs NPU 性能对比")
    print("=" * 70)

    # 注意：这里只是示例，实际需要真实的 CPU 推理数据
    # 在真实环境中，需要切换到 CPU backend 进行对比

    print("\n模拟性能数据（基于典型场景）:")
    print(f"{'指标':<20} {'CPU':<15} {'NPU':<15} {'加速比':<15}")
    print("-" * 65)
    print(f"{'推理延迟':<20} {'~2000ms':<15} {'~400ms':<15} {'5.0x':<15}")
    print(f"{'内存占用':<20} {'~3.5GB':<15} {'~1.8GB':<15} {'1.9x 更少':<15}")
    print(f"{'功耗':<20} {'~15W':<15} {'~3W':<15} {'5.0x 更低':<15}")

    print("\n✓ NPU 在性能、内存和功耗方面都有显著优势")
