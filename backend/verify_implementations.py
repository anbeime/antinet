...
try:
    ...
    print(f"  [OK] 查询团队成员成功: {len(members)}个成员")

    spaces = db_manager.get_all_knowledge_spaces()
    print(f"  [OK] 查询知识空间成功: {len(spaces)}个空间")

    activities = db_manager.get_recent_activities(limit=5)
    print(f"  [OK] 查询活动记录成功: {len(activities)}条记录")

    print("[OK] 数据库连接验证通过")
except Exception as e:
    print(f"[FAIL] 数据库连接验证失败: {e}")
    sys.exit(1)
...
try:
    ...
    print("  [OK] knowledge.py 导入成功")
    routes = [route.path for route in knowledge_router.routes]
    print(f"  [OK] 知识图谱API路由: {routes}")

    # 检查cards.py
    from api.cards import router as cards_router
    print("  [OK] cards.py 导入成功")
    routes = [route.path for route in cards_router.routes]
    print(f"  [OK] 卡片管理API路由: {routes}")

    print("[OK] API接口导入验证通过")
except Exception as e:
    print(f"[FAIL] API接口导入验证失败: {e}")
    sys.exit(1)
...
try:
    ...
    print("  [OK] MemoryAgent 导入成功")

    # 测试MemoryAgent
    memory = MemoryAgent(db_path="./data/memory_test.db")
    print(f"  [OK] MemoryAgent 初始化成功: {memory.db_path}")
    ...
    async def test_memory():
        ...
        print(f"  [OK] 知识存储成功: {result['id']}")
        return result

    asyncio.run(test_memory())

    print("  [OK] MessengerAgent 导入成功")
    print("  [OK] TaishigeAgent 导入成功")

    print("[OK] Agent实现验证通过")
except Exception as e:
    print(f"[FAIL] Agent实现验证失败: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    if 'MOCK' in knowledge_source and '# 模拟数据' in knowledge_source:
        print("  [FAIL] knowledge.py 包含模拟数据标记")
        sys.exit(1)
    else:
        print("  [OK] knowledge.py 无模拟数据残留")
    ...
    if 'MOCK' in cards_source and '# 模拟数据' in cards_source:
        print("  [FAIL] cards.py 包含模拟数据标记")
        sys.exit(1)
    else:
        print("  [OK] cards.py 无模拟数据残留")
    ...
    if 'MOCK' in memory_source and '# 模拟数据' in memory_source:
        print("  [FAIL] memory.py 包含模拟数据标记")
        sys.exit(1)
    else:
        print("  [OK] memory.py 无模拟数据残留")

    print("[OK] 无硬编码数据验证通过")
except Exception as e:
    print(f"[FAIL] 无硬编码数据验证失败: {e}")
    sys.exit(1)
...
try:
    ...
    print("  [OK] data-analysis/api/generate.py 导入成功")

    # 检查main.py中的real_inference函数
    from main import real_inference
    print("  [OK] main.real_inference 函数存在")
    ...
    print("[OK] NPU推理功能验证通过")
except Exception as e:
    print(f"[FAIL] NPU推理功能验证失败: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
