...
try:
    ...
    if _global_model_loader is None:
        print("   [ERROR] _global_model_loader 为 None")
        ...
        print(f"   [OK] _global_model_loader 已存在")
        ...
        if _global_model_loader.model is not None and not _global_model_loader.is_loaded:
            print("\n[WARNING] 检测到不一致：model exists but is_loaded=False")
            print("   尝试手动设置 is_loaded=True...")
            _global_model_loader.is_loaded = True
            print("   [OK] 已设置 is_loaded=True")
    ...
    try:
        ...
        if data.get('status') == 'healthy' and not data.get('model_loaded'):
            print("\n[WARNING] 端点仍然不一致：status='healthy' 但 model_loaded=False")
            print("   可能需要重启后端服务")
