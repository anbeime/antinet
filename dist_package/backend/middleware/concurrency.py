...
class ConcurrencyControlMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, max_concurrent_per_ip: int = 10, max_total_concurrent: int = 100, queue_timeout: float = 30.0):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self._active_requests >= self.max_total_concurrent:
            return Response(content="Service busy, please try again later", status_code=503, headers={"Retry-After": "30"})
        
        if self._ip_requests[client_ip] >= self.max_concurrent_per_ip:
            return Response(content="Too many requests from this IP", status_code=429, headers={"Retry-After": "60"})
