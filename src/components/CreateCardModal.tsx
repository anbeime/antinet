...
interface CreateCardModalProps {
  ...
  existingCards: { id: string; title: string; content: string }[];
  projectId?: number;
  projectName?: string;
}
...
const CreateCardModal: React.FC<CreateCardModalProps> = ({ 
  ...
  const extractKeywords = (text: string): string[] => {
    ...
    return [...new Set(words.filter(w => w.length > 1 && w.length < 20))];
  };

  // 自动匹配关联卡片
  const autoSuggestRelated = (content: string) => {
    if (!content.trim() || existingCards.length === 0) return;
    const keywords = extractKeywords(content);
    const scored = existingCards
      .filter(c => !formData.relatedCards.includes(c.id))
      .map(c => {
    ...
    if (scored.length > 0) {
      setSuggestedRelated(scored);
    } else {
      setSuggestedRelated([]);
    }
  };

  const [suggestedRelated, setSuggestedRelated] = useState<{ id: string; title: string; score: number }[]>([]);
  ...
  const handleContentChange = (value: string) => {
    ...
    autoSuggestTimer = setTimeout(() => autoSuggestRelated(value), 500);
  };
  let autoSuggestTimer: ReturnType<typeof setTimeout> | undefined;
;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // 从内容中自动提取来源地址
    if (name === 'content' && !formData.address) {
      const urlMatch = value.match(/https?:\/\/[^\s\n\)\]]+/);
      if (urlMatch) {
        const url = urlMatch[0].replace(/[\)\]]$/, '').slice(0, 120);
        setFormData(prev => ({ ...prev, address: url }));
      }
    }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            {/* 自动推荐关联（基于内容关键词匹配） */}
            {suggestedRelated.length > 0 && formData.relatedCards.length < 5 && (
              <div className="mt-3">
                <p className="text-xs text-gray-400 mb-2">推荐关联（基于内容匹配）</p>
                <div className="flex flex-wrap gap-2">
                  {suggestedRelated.map(item => (
                    <button
                      key={item.id}
                      type="button"
                      onClick={() => addRelatedCard(item.id)}
                      className="inline-flex items-center px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full text-sm hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
                    >
